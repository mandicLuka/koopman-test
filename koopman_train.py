...
def train_and_save_model(model_name, dataset_name, train_params, push_xcom=False, **context):
    ...
    history.model.save(os.path.join(dir_path, train_params["save_path"], \
        f"{model_name}-{dataset_name}-{datetime.now()}"))
    if push_xcom:
        task = context["ti"]
